...
const THOUGHT_PATTERN = /(?:^(?:想法|点子|idea)\s*[：:]|(?:记下|记录|保存|新增|添加)(?:一个|这条|一下)?(?:想法|点子)|(?:把|将).{0,40}(?:记为|存为)(?:想法|点子))/i
...
const READ_ONLY_TASK_PATTERN = /(?:检查|查看|看看|排查|诊断|分析|审查|review|inspect|check|analy[sz]e)/i
const MUTATION_TASK_PATTERN = /(?:修复|修改|改一下|改下|调整|优化|实现|新增|添加|删除|移除|重构|打包|安装|构建|运行|测试|部署|发布|提交|创建|生成|更新|配置|接入|fix|change|modify|update|implement|add|remove|delete|refactor|build|test|run|deploy|install|package|create|configure|integrate)/i
...
export function routeAgentChatInput(input: string, context: RouteContext): AgentChatRoute {
  ...
  if (THOUGHT_PATTERN.test(prompt)) return { kind: 'thought' }
...
export function inferAgentTaskIntent(input: string): AgentTaskIntent {
  ...
  return /(?:只读|不修改|不要修改|无需修改|不改动|仅查看|仅检查|read[- ]only|without (?:changing|modifying))/i.test(prompt)
    || (READ_ONLY_TASK_PATTERN.test(prompt) && !MUTATION_TASK_PATTERN.test(prompt))
    ? 'analysis'
    : 'change'
}
