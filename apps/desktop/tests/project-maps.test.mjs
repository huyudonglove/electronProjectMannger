...
test('task map groups persistent multi-round runs under their project task', () => {
  ...
  const runs = [
    { runId: 'run-2', status: 'completed', task, updatedAt: '2026-01-02T00:00:00.000Z', stepCount: 4, eventSequence: 12, progress: { changedFiles: ['b.ts'], verificationPassed: 1, verificationFailed: 0 }, diff: { summary: '第二轮完成' }, diagnostics: { rejectedActions: 1, failedModelAttempts: 1, recentErrors: [{ sequence: 8, at: '2026-01-02T00:00:00.000Z', type: 'model.rejected', phase: 'acting', summary: '动作无效' }] } },
  ...
  const result = projectMapsView(codeMap, { tasks: [task] }, [], runs)
