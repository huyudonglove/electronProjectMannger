...
try {
  ...
  assert(brief.instructions.some((instruction) => instruction.includes('Txxx/Ixxx/Dxxx/Qxxx/Rxxx/Lxxx/Cxxx/Vxxx')), 'brief should describe descending aggregate record write order')
  ...
  assert(brief.instructions.some((instruction) => instruction.includes('简单明确的 light 工作直接执行')), 'brief should keep simple work direct')
  assert(brief.instructions.some((instruction) => instruction.includes('目标和验收是稳定锚点')), 'brief should preserve the goal while plans change')
  assert(brief.instructions.some((instruction) => instruction.includes('轻量检查不足时才扩大到全量测试')), 'brief should prefer focused verification')
  assert(brief.instructions.some((instruction) => instruction.includes('可并行或适合专项调查的工作交给子 Agent')), 'brief should allow efficient subagent delegation')
...
`, 'utf8')
  ...
  assert(updatedSkill.includes('Execute clear light work directly'), 'guidance update should keep simple work direct')
  assert(updatedSkill.includes('goal and acceptance criteria are the stable anchor'), 'guidance update should preserve goals across plan changes')
  assert(updatedSkill.includes('Do not preserve low-value plan churn'), 'guidance update should avoid persisting planning noise')
  assert(updatedSkill.includes('Expand to the full suite only'), 'guidance update should prefer focused verification')
  assert(updatedSkill.includes('When the runtime supports subagents'), 'guidance update should allow subagents when they improve throughput')
  assert(updatedSkill.includes('main agent retains the goal'), 'guidance update should keep integration ownership with the main agent')
  ...
  assert(updatedDataSpec.includes('Txxx') && updatedDataSpec.includes('Ixxx') && updatedDataSpec.includes('Dxxx') && updatedDataSpec.includes('Qxxx') && updatedDataSpec.includes('Rxxx') && updatedDataSpec.includes('Lxxx') && updatedDataSpec.includes('Cxxx') && updatedDataSpec.includes('Vxxx'), 'data spec should describe descending aggregate record write order')
  ...
  assert(updatedDataSpec.includes('简单明确的 light 工作直接执行'), 'data spec should keep simple work direct')
  assert(updatedDataSpec.includes('目标和验收始终是稳定锚点'), 'data spec should anchor dynamic plans to the goal')
  assert(updatedDataSpec.includes('环境支持子 Agent 时'), 'data spec should describe efficient subagent delegation')
